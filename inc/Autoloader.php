...
class Autoloader {
	 ...
	 * @var string
	 */
	const TOOLBOX_NAMESPACE_PREFIX = 'ToolboxModules\\';
	...
	private function is_class_in_autoloaded_namespace( $class ) {
		$prefix_length = strlen( self::TOOLBOX_NAMESPACE_PREFIX );

		return 0 === strncmp( self::TOOLBOX_NAMESPACE_PREFIX, $class, $prefix_length );
	}
	...
	private function get_class_file_path( $class ) {
		$relative_class_path = substr( $class, strlen( self::TOOLBOX_NAMESPACE_PREFIX ) );
		$relative_file_path = str_replace( '\\', '/', $relative_class_path ) . '.php';

		return dirname( CHECK_TOOLBOXMODULES_PLUGIN_FILE ) . '/inc/' . $relative_file_path;
	}
